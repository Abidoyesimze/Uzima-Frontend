...
export default function InstallPrompt() {
  const { deferredPrompt, showPrompt, isIos, handleInstall, dismissPrompt } = usePwaInstall();
  ...
  if (showPrompt && isIos) {
    return (
      <div className="fixed bottom-4 left-4 right-4 bg-white border rounded-md shadow-lg p-4">
        ...
        <button className="mt-2 px-4 py-2 bg-gray-200 rounded" onClick={dismissPrompt}>
          Dismiss
        </button>
      </div>
    );
